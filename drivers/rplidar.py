...
class RPLidar:
    def __init__(self, zeroros_publisher, params, parent=None):
        ...
        self.zeroros_publisher = zeroros_publisher
        self.current_scan = []

        self.stop_event = threading.Event()
        self.th = threading.Thread(
            target=self.loop, args=[self.stop_event], daemon=True
        )
        self.th.start()

    def publish(self, current_scan):
        self.msg.header.stamp = datetime.datetime.utcnow().timestamp()
        # Angles go from zero to 2π. Sort by angle
        sorted_data = sorted(current_scan, key=lambda x: x[0])
        angles, ranges, intensities = zip(*sorted_data)
        
        self.msg.ranges = np.array(ranges)
        self.msg.angles = np.array(angles)
        self.msg.intensities = np.array(intensities)
        self.zeroros_publisher.publish(self.msg)
        ...
        try:
            while not stop_event.wait(1):
                ...
                if len(polled_samples) > 0:
                    ...
                    for angle, range, quality, new_scan in polled_samples:
                        angle = np.radians(angle + 180.0)
                        angle = np.arctan2(np.sin(angle), np.cos(angle))
                        range = np.array(range) / 1000.0

                        if new_scan and len(self.current_scan) >= 120:
                            self.publish(self.current_scan)
                            self.current_scan = []
                            
                        if (angle > self.angle_min_rad) & (angle < self.angle_max_rad):
                            if (range < self.range_min_m) | (range > self.range_max_m):
                                range = np.nan
                                quality = np.nan
                            self.current_scan.append((angle, range, quality))
