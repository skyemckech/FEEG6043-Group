#%% emperical calculation of R matrix
import json
import numpy as np
import math
...
def extract_variables(filepath):
    ...
    northings_ground = variables.extract_data("/aruco", ["message", "pose","position", "x"])
    eastings_ground = variables.extract_data("/aruco", ["message", "pose","position", "y"])
    heading_x = variables.extract_data("/aruco", ["message","pose", "orientation", "x"])
    heading_y = variables.extract_data("/aruco", ["message","pose", "orientation", "y"])
    heading_z = variables.extract_data("/aruco", ["message","pose", "orientation", "z"])
    heading_w = variables.extract_data("/aruco", ["message", "pose","orientation", "w"])
    ...
    timestamps = variables.extract_data("/aruco", ["timestamp"])
...
filepaths = ["logs/20250303_190300_log.json", "logs/20250303_190345_log.json", "logs/20250303_190426_log.json", "logs/20250303_190506_log.json", "logs/20250303_190632_log.json"]
...
for key in ["northings_ground", "eastings_ground", "headings", "angular_velocity"]:
...
for i in range(min_length):
    ...
    variances["forward_velocity"].append(np.var(forward_velocity_values, ddof=1))
...
plt.subplot(2, 3, 4)
plt.plot(time, variances["forward_velocity"], label="Forward Velocity", color='red')
plt.title("Variance of Forward Velocity")
plt.xlabel("Time (s)")
plt.ylabel("Variance")
